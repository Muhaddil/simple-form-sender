...
async function handleSubmit() {
  ...
  const payloadSections = [
    ...
    `- **¿Cómo actuarías en un rescate?:** ${medicationForInfection.value}`,
...
async function sendMessageToWebhook(content: string, username: string, avatar_url: string) {
  if (!webhook || webhook.trim() === '') {
    throw new Error('El webhook de Discord no está configurado.');
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(`Error en Discord: ${response.status} - ${text}`);
  }
}
...
<template>
  <div class="container">
    <header>
      ...
      <h1 class="title">Formulario SAFD — 1</h1>
    ...
    <div class="form-container">
      ...
      >
        ...
        >
          ...
          <div
            v-if="currentStep === 1"
            class="form-section active-section"
            key="step1"
          >
          ...
          >
            ...
            <div class="form-field">
              <label for="emsRoleKnowledge">Conocimiento del rol de SAFD</label>
              <textarea
                id="emsRoleKnowledge"
            ...
            <div class="form-field">
              <label for="pseudoICTerm"
                >¿Por qué deberíamos elegirte?</label
              >
              <textarea
                id="pseudoICTerm"
                v-model="whyChooseMe"
          ...
          >
            <h3 class="section-title">
              ...
              Conocimientos de Bomberos y Rescate
            </h3>

            <div class="form-field">
              <label for="treatmentForInjury">¿Como actuarias en un rescate?</label>
              <textarea
          ...
          >
            ...
            <div class="form-field">
              <label for="defineEnvironment">Define DM</label>
              <textarea
                id="defineEnvironment"
                v-model="defineDM"
                rows="2"
                placeholder="Define con tus palabras DM"
                required
              ></textarea>
            </div>

            <div class="form-field">
              <label for="canUseVoiceMods">Define con tus palabras el PG</label>
              <textarea
                id="canUseVoiceMods"
                v-model="definePG"
                rows="2"
                placeholder="Define con tus palabras el PG"
                required
              ></textarea>
            </div>
